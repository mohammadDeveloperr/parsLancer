<template>
    <div>
        ...
        <sidebar :sidebar-items="sidebarItems"></sidebar>
        ...
        <div id="projectPage">
                        ...
                        <b-card v-for="project in filteredProjects" :key="project.id" class="mb-4 project-card">
                            <b-card-body class="text-right">
                                ...
                                <div class="d-flex justify-content-between align-items-center mt-3" dir="rtl">
                                    ...
                                    <b-button class="mt-5" variant="success" @click="viewProject(project.id)">مشاهده
                                        پروژه</b-button>
            ...
            <b-pagination v-model="currentPage" :total-rows="totalProjects" :per-page="perPage"
                :aria-controls="filteredProjects" class="my-4 pagination"></b-pagination>
        </div>
    </div>
...
<script>
/* eslint-disable */
import projectCards from './projectsCards.vue'
...
export default {
        ...
        async fetchProject() {
            try {
                ...
                let config = {
                    ...
                    url: `http://localhost:3000/suggest/${this.user.username}`,
                    headers: { token: this.token }
                    // params: {freelancer_username:this.user.username}
                };
                ...
                this.projects = response.data.suggests
            } catch (err) {
                console.log(err)
                this.projects = []
            }
    ...
    computed: {
       ...
       const result=filteredProjects.slice(start, end);
